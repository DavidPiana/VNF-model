...
DATA_DIR = Path(__file__).parent / "Data" / "norway"
...
def build(num_demands: int | None, start_demand: int = 0, aif_nodes: list[str] = []):
    # Leggi topologia
    inside = (DATA_DIR / "inside_topology_norway").read_text()
    V = parse_set(inside, "V")
    AIF = set(aif_nodes)
    A_links = parse_arc_set(inside, "A_links")
    lv = {node: 2 if node in AIF else 1 for node in V}
    
    # Leggi delay
    delay_data = parse_param_matrix((DATA_DIR / "delay_norway.dat").read_text(), "delay_link")
    ...
    # Leggi SNDLIB per le domande
    sndlib_text = (DATA_DIR / "SNDLIB_norway").read_text()
...
def main():
    ...
    parser.add_argument("--aif", nargs='+', default=[])
    parser.add_argument("--out", type=str, required=True)
    args = parser.parse_args()
    
    data = build(num_demands=args.num_demands, start_demand=args.start_demand, aif_nodes=args.aif)
