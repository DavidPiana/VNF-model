...
# big-M as derived in the formulation: M = max_f sigma_f + max_f gamma_f * |D|
param M := max {f in F} sigma[f] + (max {f in F} gamma[f]) * card(D);
...
# (11) processing time per (node, VNF) instance
subject to ProcTime {i in N, f in F: alpha[i,f] = 1}:
