...
#  Objective: minimize the sum of end-to-end latencies (L)
#
#  Rispetto a vnf_model.mod, questo file aggiunge:
#    - Filtraggio di w[i,u,v,s] in base ad alpha: w esiste solo se il
#      nodo i puo' eseguire chain[s] (alpha[i,chain[s]]=1).
#    - Coupling e NoBifurcation filtrati di conseguenza.
#    - Riduzione ~66% delle variabili w e dei vincoli Coupling/NoBifurcation.
#    - FUNZIONE OBIETTIVO MODIFICATA: minimizza la somma delle latenze anziche il massimo.
#
...
# --------------------------- OBJECTIVE ---------------------------------
minimize Obj_SumLatency: sum {k in D} L[k];
...
subject to ProcTimeOther {i in N, f in F: f != 'F2' and alpha[i,f] = 1 and Mf[f] > 0}:
    T[i,f] = sigma[f]*y[i,f] + gamma[f]*sum {k in D} z[k,i,f];

# (12)-(13) linearisation of Theta_{i,f}^k
subject to Theta1 {k in D, i in N, f in F: alpha[i,f] = 1 and Mf[f] > 0}:
    Theta[k,i,f] <= Mf[f] * z[k,i,f];

subject to Theta2 {k in D, i in N, f in F: alpha[i,f] = 1 and Mf[f] > 0}:
...
subject to ThetaZero {k in D, i in N, f in F: alpha[i,f] = 0 or Mf[f] == 0}:
    Theta[k,i,f] = 0;
